...
export default async function GuardianStudentPermissionsPage({
  params,
}: Readonly<{ params: Promise<{ linkId: string }> }>) {
  noStore();
  const { linkId } = await params;
  const service = await getGuardianPortalService();
  const permissions = await service.getStudentPermissions(linkId);

  return (
    <GuardianPortalSection title="Permisos visibles">
